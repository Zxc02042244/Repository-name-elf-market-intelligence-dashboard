...
test("Cosmic Sovereign three-piece frame stays aligned", async ({ page }, testInfo) => {
  await page.goto("/?v=playwright-cosmic-sovereign#home", {
    waitUntil: "domcontentloaded"
  });
  await page.waitForTimeout(1_200);

  const skinButton = page.getByRole("button", { name: /Cosmic Sovereign/ });
  await expect(skinButton).toHaveCount(1);
  await skinButton.click();

  const card = page.locator('[data-view="desktop"] .elf-champion-frame-cosmic-sovereign');
  ...
  await card.screenshot({
    path: testInfo.outputPath("cosmic-sovereign-card.png"),
    animations: "disabled"
  });
});
