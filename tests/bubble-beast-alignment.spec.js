...
test("Bubble Beast three-piece frame stays aligned", async ({ page }, testInfo) => {
  await page.goto("/?v=playwright-bubble-beast#home", {
    waitUntil: "domcontentloaded"
  });
  await page.waitForTimeout(1_200);

  const skinButton = page.getByRole("button", { name: /Bubble Beast/ });
  await expect(skinButton).toHaveCount(1);
  await skinButton.click();

  const card = page.locator('[data-view="desktop"] .elf-champion-frame-bubble-beast');
  ...
  await card.screenshot({
    path: testInfo.outputPath("bubble-beast-card.png"),
    animations: "disabled"
  });
});
