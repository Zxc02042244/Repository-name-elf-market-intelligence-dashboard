...
test("Genesis Pioneer three-piece frame stays aligned", async ({ page }) => {
  ...
  await page.route("**/api/v1/elf/skins**", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        code: 0,
        skins: [
          { skinName: "Genesis Pioneer", skinUrl: transparentPixel, quantity: 999 },
  ...
  await page.goto("/?v=playwright-genesis-pioneer#home", { waitUntil: "domcontentloaded" });
  await page.waitForTimeout(1_200);
  await page.locator("[data-skin-home-tab='supply']").first().click();

  const card = page.locator('[data-view="desktop"] .elf-champion-frame-genesis-pioneer');
  ...
  expect(alignment.rankBackground).toContain("genesis-pioneer-rank-plaque-v1.png");
  expect(alignment.nameBackground).toContain("genesis-pioneer-name-plaque-v1.png");
  expect(alignment.nameOutsideContainer).toBe(false);
  await expect(rank).toHaveText("TOP 1");
  await expect(name).toHaveText("Genesis Pioneer");
});
