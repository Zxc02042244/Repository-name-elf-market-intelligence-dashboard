...
test("Starborn Warrior three-piece frame stays aligned", async ({ page }, testInfo) => {
  await page.goto("/?v=playwright-starborn-warrior#home", {
    waitUntil: "domcontentloaded"
  });
  await page.waitForTimeout(1_200);

  const skinButton = page.getByRole("button", { name: /Starborn Warrior/ });
  ...
  await card.screenshot({
    path: testInfo.outputPath("starborn-warrior-card.png"),
    animations: "disabled"
  });
});
