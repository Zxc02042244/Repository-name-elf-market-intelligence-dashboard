...
test("Pumpkin Whisper three-piece frame stays aligned", async ({ page }) => {
  ...
  await page.route("**/api/v1/elf/skins**", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        code: 0,
        skins: [
          { skinName: "Pumpkin Whisper", skinUrl: transparentPixel, quantity: 999 },
  ...
  await page.goto("/?v=playwright-pumpkin-whisper#home", { waitUntil: "domcontentloaded" });
  await page.waitForTimeout(1_200);
  await page.locator("[data-skin-home-tab='supply']").first().click();

  const card = page.locator(".elf-tab-panel-supply .elf-champion-frame-pumpkin-whisper").last();
  ...
  expect(alignment.rankBackground).toContain("pumpkin-whisper-rank-plaque-v1.png");
  expect(alignment.nameBackground).toContain("pumpkin-whisper-name-plaque-v1.png");
  expect(alignment.nameOutsideContainer).toBe(false);
  await expect(rank).toHaveText("TOP 1");
  await expect(name).toHaveText("Pumpkin Whisper");
});
