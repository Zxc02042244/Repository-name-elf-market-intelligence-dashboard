...
test("Shark Hoodie three-piece frame stays aligned", async ({ page }) => {
  ...
  await page.route("**/api/v1/elf/skins**", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        code: 0,
        skins: [
          { skinName: "Shark Hoodie", skinUrl: transparentPixel, quantity: 999 },
  ...
  await page.goto("/?v=playwright-shark-hoodie#home", { waitUntil: "domcontentloaded" });
  ...
  expect(alignment.rankBackground).toContain("shark-hoodie-rank-plaque-v1.png");
  expect(alignment.nameBackground).toContain("shark-hoodie-name-plaque-v1.png");
  expect(alignment.nameOutsideContainer).toBe(false);
  await expect(rank).toHaveText("TOP 1");
  await expect(name).toHaveText("Shark Hoodie");
});
