...
test("Zombie Walker three-piece frame stays aligned", async ({ page }, testInfo) => {
  await page.goto("/?v=playwright-zombie-walker#home", {
    waitUntil: "domcontentloaded"
  });
  await page.waitForTimeout(1_200);

  const skinButton = page.getByRole("button", { name: /Zombie Walker/ });
  await expect(skinButton).toHaveCount(1);
  await skinButton.click();

  const card = page.locator('[data-view="desktop"] .elf-champion-frame-zombie-walker');
  ...
  await card.screenshot({
    path: testInfo.outputPath("zombie-walker-card.png"),
    animations: "disabled"
  });
});
